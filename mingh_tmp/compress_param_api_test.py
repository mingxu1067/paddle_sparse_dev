...
def main():
    ...
    with fluid.program_guard(train_program, startup_prog):
        ...
        fc = sparsity.fc_sparse(input=input_data, num_flatten_dims=-1, size=32, act=None)
    ...
    fc_result = exe.run(
            train_program, feed=feeder.feed([(data,)]), fetch_list=[fc])[0]

    sparsity.ASPHelper.compress_model(train_program, place)

    fc_sparse_compressed_result = exe.run(
            train_program, feed=feeder.feed([(data,)]), fetch_list=[fc])[0]

    for i in range(32):
        for j in range(32):
            if fc_result[i][j] != fc_sparse_compressed_result[i][j]:
                        print(i, j, "::", fc_result[i][j], "-" ,fc_sparse_compressed_result[i][j])
